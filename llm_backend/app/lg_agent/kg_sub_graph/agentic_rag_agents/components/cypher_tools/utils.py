...
async def validate_cypher_query_with_llm(
    ...
    if llm_output.filters:
        for filter in llm_output.filters:
            #安全检查：确保 structured_schema包含 node_props
            if "node_props" not in graph.structured_schema:
                # 如果没有 node_props，跳过属性类型检查
                continue
...
def create_text2cypher_generation_node(
    ...
    logging.info("Creating Text2Cypher 生成节点...")
    print("正在创建 Text2Cypher 生成节点...")
    async def generate_cypher(state: CypherInputState) -> Dict[str, Any]:
        ...
        print(f"生成Cypher: {generated_cypher}")
...
def create_text2cypher_validation_node(
    ...
    async def validate_cypher(state: CypherState) -> Dict[str, Any]:
        ...
        # 确保 corrected_cypher 是字符串类型
        if isinstance(corrected_cypher, str):
            current_cypher = corrected_cypher
        else:
            current_cypher = cypher_statement
        ...
        if not llm_validation:
            cypher_errors = validate_cypher_query_with_schema(
                graph=graph, cypher_statement=current_cypher
            )
        ...
        # 确保返回的statement是字符串而不是协程对象
        final_statement = corrected_cypher if isinstance(corrected_cypher, str) else current_cypher
...
def create_text2cypher_execution_node(
    ...
    async def execute_cypher(
        ...
        # 提取customerId参数
        import re
        customer_match = re.search(r'客户[\s]*ID[\s]*[:：]?[\s]*(\d+)', user_question)
        if customer_match:
            parameters["customerId"] = customer_match.group(1)
        else:
            # 如果没有明确指定客户ID，使用默认值
            parameters["customerId"] = "1001"  # 默认客户ID
        
        # 执行带参数的查询
        records = graph.query(cypher_statement, parameters)
