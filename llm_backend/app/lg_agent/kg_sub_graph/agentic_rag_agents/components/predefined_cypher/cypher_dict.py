...
predefined_cypher_dict: Dict[str, str] = {
    ...
    "customer_orders": "MATCH (c:Customer)-[:PLACED_BY]->(o:Order) WHERE c.UserID = $customer_id RETURN o.OrderID, o.OrderDate, o.ShippedDate, o.Status, o.TotalAmount ORDER BY o.OrderDate DESC",
    "customer_purchase_history": "MATCH (c:Customer)-[:PLACED_BY]->(o:Order)-[:CONTAINS]->(p:Product) WHERE c.UserID = $customer_id RETURN p.ProductName, o.OrderDate, p.UnitPrice ORDER BY o.OrderDate DESC",
    "my_orders": "MATCH (c:Customer {UserID: $customer_id})-[:PLACED_BY]->(o:Order) RETURN o.OrderID, o.OrderDate, o.Status, o.TotalAmount ORDER BY o.OrderDate DESC",
    "order_list": "MATCH (c:Customer {UserID: $customer_id})-[:PLACED_BY]->(o:Order) RETURN o.OrderID as orderId, o.OrderDate as orderDate, o.Status as status, o.TotalAmount as totalAmount ORDER BY o.OrderDate DESC",
    
    #订单类查询
    "order_by_id": "MATCH (o:Order) WHERE o.OrderID = $order_id RETURN o.OrderDate, o.RequiredDate, o.ShippedDate, o.Status, o.TotalAmount",
    ...
    # 用户行为查询
    "user_behavior_history": "MATCH (ub:UserBehavior) WHERE ub.UserID = $customer_id RETURN ub.ProductID, ub.BehaviorType, ub.Timestamp ORDER BY ub.Timestamp DESC",
    "user_purchase_history": "MATCH (ub:UserBehavior) WHERE ub.UserID = $customer_id AND ub.BehaviorType = 'purchase' RETURN ub.ProductID, ub.Timestamp ORDER BY ub.Timestamp DESC"
}