...
包括但不限于：
- 商品价格、库存、规格等信息查询
-订单状态、物流信息查询（包含"我的订单"、"订单列表"、"查询订单"等）
- 会员积分、优惠活动查询
- 退换货政策咨询
- 商品使用指导及故障解决方法
- 用户行为历史查询
...
## 重要规则：
- 当用户询问"我的订单"、"订单列表"、"查询订单"等时，必须分类为`graphrag-query`
- 当用户询问个人购物历史、购买记录时，必须分类为`graphrag-query`
-所有涉及用户个人数据的查询都应走`graphrag-query`分支
"""
...
4. 表达歉意和继续服务意愿："如果您有任何关于我们商品或服务的问题，随时欢迎询问哦～❤️"
