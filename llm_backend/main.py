...
async def langgraph_query(
    ...
    try:
        ...
        # 检查是否请求人工客服
        if any(keyword in query for keyword in ["人工客服", "转人工", "客服","人工"]):
            # 返回人工客服信息
            human_service_info = {
                "type": "human_service",
                "message": "正在为您转接人工客服...",
            ...
            #直返回人工客服信息，不走LangGraph流程
            thread_id = conversation_id if conversation_id else new_uuid()
            return JSONResponse(
                content=human_service_info,
                headers={"X-Conversation-ID": thread_id}
            )
        ...
        if image and image.filename:
            ...
            # 生成带时间戳的文件名
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            original_name, ext = os.path.splitext(image.filename or "")
            new_filename = f"{original_name}_{timestamp}{ext}"
